...
def grid_search(num_samples: int, min: float = None, max: float = None, **kwargs)->Iterable:
    ...
    Example use: hparam_ranges['batch_size'] = grid_search(num_samples=6, min=64, max=512, log=True)
...
def random_search(num_samples: int, distribution: str, min: float=None, max: float=None, **kwargs) -> Iterable:
    ...
    Example use: hparam_ranges['lr'] = random_search(num_samples=10, distribution='exponential', min=1e-6, max=1e-1, rate=0.1)
